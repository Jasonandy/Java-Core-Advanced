 ...
 *  <p> Created on 2018年7月9日 下午4:33:24</p>
...
* @Description：   <p> ConcurrentHashMap </p>
* {@link https://www.cnblogs.com/chengxiao/p/6842045.html}
* {@link https://www.cnblogs.com/study-everyday/p/6430462.html}
* @Author： - Jason   
* @CreatTime：2018年7月9日 下午4:33:24   
* @Modify By：   
* @ModifyTime：  2018年7月9日
* @Modify marker：   
* @version    V1.0
*/
/****************************************************************************************************
...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V> implements ConcurrentMap<K,V>, Serializable {
	...
	private static final long serialVersionUID = -6235526205538462171L;

    /* ---------------- Constants -------------- */

	/**
	 * 最大可能的表容量 这个值必须正好1< 30，以保持在Java数组分配和索引中 两个表大小的幂次的界限
	 * 并进一步要求因为32位哈希字段的前两个位用于控制目的。
	 */
    private static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * 默认初始表容量 一定是2的幂(至少是1)和最多最大容量
     */
    private static final int DEFAULT_CAPACITY = 16;

    /**
     * 最大的(非两倍的)数组大小 和toArray方法相关
     */
     ...
     * @Package：cn.ucaner.sourceanalysis.map   
     * @ClassName：Node   
     * @Description：   <p> 一个链表，但是只允许对数据进行查找，不允许进行修改 </p>
     * @Author： - Jason   
     * @CreatTime：2018年7月9日 下午4:58:28   
     * @Modify By：   
     * @ModifyTime：  2018年7月9日
     * @Modify marker：   
     * @version    V1.0
     ...
     * @Package：cn.ucaner.sourceanalysis.map   
     * @ClassName：Segment   
     * @Description：   <p> Segment 分段</p>
     * @Author： - Jason   
     * @CreatTime：2018年7月9日 下午4:49:09   
     * @Modify By：   
     * @ModifyTime：  2018年7月9日
     * @Modify marker：   
     * @version    V1.0
    ...
    /**
	 * @Description:step1. 如果没有初始化就先调用initTable()方法来进行初始化过程 
	 * @return Node<K,V>[]
	 * @Autor: Jason
	 */
    ...
    /**
	 * @Description: Step6. 表示已经数据加入成功了，现在调用addCount()方法计算ConcurrentHashMap的size，在原来的基础上+1
	 * @param x
	 * @param check void
	 * @Autor: Jason
	 */
      ...
      //更新baseCount，table的数量，counterCells表示元素个数的变化
        if ((as = counterCells) != null ||
            ...
            if (as == null || (m = as.length - 1) < 0 ||
            	//TODO ThreadLocalRandom.getProbe FIX
    ...
    /**
     * @Description: helpTransfer 
     * Step2. 如果没有hash冲突就直接CAS插入, 从旧的table的元素复制到新的table中
     * @param tab
     * @param f
     * @return Node<K,V>[]
     * @Autor: Jason
    ...
    /**
     * @Description: 调用多个工作线程一起帮助进行扩容
     * 这样的效率就会更高，而不是只有检查到要扩容的那个线程进行扩容操作，其他线程就要等待扩容操作完成才能工作
     * @param tab
     * @param nextTab void
     * @Autor: Jason
     */
    ...
    /**
	 * @Description: 向链表或者红黑树里加节点，到第五步，会调用treeifyBin()方法进行链表转红黑树的过程
	 * @param tab
	 * @param index void
	 * @Autor: Jason
	 */
     ...
     * @Package：cn.ucaner.sourceanalysis.map   
     * @ClassName：TreeNode   
     * @Description：   <p> TreeNode继承与Node</p>
     * 但是数据结构换成了二叉树结构，它是红黑树的数据的存储结构，
     * 用于红黑树中存储数据，当链表的节点数大于8时会转换成红黑树的结构，
     * 通过TreeNode作为存储结构代替Node来转换成黑红树.
     * @Author： - Jason   
     * @CreatTime：2018年7月9日 下午4:59:31   
     * @Modify By：   
     * @ModifyTime：  2018年7月9日
     * @Modify marker：   
     * @version    V1.0
    ...
    /* ---------------- TreeBins -------------- */
  	...
  	* @Package：cn.ucaner.sourceanalysis.map   
  	* @ClassName：TreeBin   
  	* @Description：   <p> TreeBin </p>
  	* @Author： - Jason   
  	* @CreatTime：2018年7月9日 下午5:00:46   
  	* @Modify By：   
  	* @ModifyTime：  2018年7月9日
  	* @Modify marker：   
  	* @version    V1.0
